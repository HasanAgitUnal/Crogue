...
--- @class _CONTAINER<T>
--- @field [integer] T
--- @field size fun(): integer                          Returns the number of elements in the container.
--- @field set fun(key: integer, value: T)              Sets a value at the given index. If value is nil, erases the element at that index. If index is size+1, inserts the value at the end.
--- @field at fun(key: integer): T                      Returns the value at the given index. For non‑random‑access containers, this may return multiple values.
--- @field get fun(key: integer): T                     Returns the value at the given index (same as `at`). For non‑random‑access containers, this may return multiple values.
--- @field find fun(target: T): boolean                 Finds a value in the container. (For sequence containers, this performs a linear search.)
--- @field erase fun(target: integer)                   Erases the element at the given index. (For sequence containers, the target is an index.)
--- @field insert fun(target: integer, value: T)        Inserts a value at the specified index. (For sequence containers, the target is an index.)
--- @field add fun(value: T)                            Adds a value to the end of the container.
--- @field clear fun()                                  Removes all elements from the container.
--- @field pairs fun(): fun(), _CONTAINER<T>, nil       Lua 5.2+ only; use c:pairs() in Lua 5.1/LuaJIT.
--- @field ipairs fun(): fun(), _CONTAINER<T>, integer  Lua 5.2+ only; use c:ipairs() in Lua 5.1/LuaJIT.
