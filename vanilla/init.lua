-- Settings --
local settings = cr.settings("vanilla")

local teleporter_enabled = settings.enable_teleporter
local zombie_name = settings.zombie_name
local mode = settings.my_mode

local add_spider = mode == "Normal" and mode == "Hard"
local add_apple = mode == "Easy"
                        ...
                        for _, slot in ipairs(slots) do
                                if slot.front and slot.front.name ~= "~ Exit Gate ~" then
                                        ...
                                        if #cr.stat.card_set == 0 then
                                                -- make nil if no card to replace
                                                slot.back = nil
                                        else
